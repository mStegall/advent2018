fn main() {
    ...
    let processed_lines = data.lines().map(|line| {
        ...
        return (coord_x, coord_y, dimension_x, dimension_y, id);
    });

    processed_lines.clone().for_each(|line| {
        let (coord_x, coord_y, dimension_x, dimension_y, id) = line;
    ...
    let uncontested_id = processed_lines.clone().find(|line| {
        let (coord_x, coord_y, dimension_x, dimension_y, id) = line;
        ...
        for x in 0..*dimension_x {
            for y in 0..*dimension_y {
                if table[coord_x + x][coord_y+y] > 1 {
                    return false
                };
            }
        }
