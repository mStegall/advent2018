...
pub fn main() {
            ...
            if !awake {
                ...
                match guards.get(&guard_number_active) {
                    ...
                    None => {
                        guards.insert(
                            guard_number_active,
                            count_minutes([0; 60], asleep_time, time),
                        );
                    }
                }
    ...
    for (key, val) in guards.iter() {
        let total_sleep: i32 = *val.iter().max().unwrap();
    ...
    let best_guard_minute = best_guard.iter().enumerate().max_by_key(|a| a.1).unwrap();

    let answer = best_guard_minute.0 as isize * best_guard_id as isize;
    println!("{answer}")
}
